...
class VisionTargetDetector2:
	...
	def __init__(self, input):
		...
		self.DISTANCE_FROM_CORNER_TO_CENTER = (2.54/100)np.sqrt(16 + np.square((5.5*np.cos(14.5(np.pi/180)) + 2*np.sin(14.5(np.pi/180)))/2 - 2*np.sin(14.5(np.pi/180))) ) #add here after calculati
	...
	def run_cv(self):
		...
		for pair in self.get_all_pairs(rotated_boxes):
			...
			dist_to_cent = depth_frame.get_distance(int(pair.get_center().x), int(pair.get_center().y))
			
			

			left = pair.get_left()
			right = pair.get_right()
			dist_to_point = depth_frame.get_distance(int(left.get_p2().x), int(left.get_p2().x))

			angle_off_perp = np.arccos((np.square(dist_to_point) - np.square(dist_to_cent) - np.square(self.DISTANCE_FROM_CORNER_TO_CENTER))/(-2*self.DISTANCE_FROM_CORNER_TO_CENTER*dist_to_cent))

			print("dist: ", str(dist_to_cent))
			print("angle: ", str(angle_off_perp*(180/np.pi)))
